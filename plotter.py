...
def scatter_entry_hour(df):
    # Create a list by summing (appending) all lists of entry hour
    all_entry_hour = df['nodes_entry_hour'].sum()
    # Count how many nodes have entered in a certain time interval
    d1 = pd.value_counts(all_entry_hour).to_dict()
    plt.scatter(d1.keys(), d1.values(), color='b')
    
    #entries = df['brokers_entry_time'].sum()
...
if __name__ == "__main__":
    ...
    day_index = 0                                 # 28 April
    # bridges_brokers_cc(df)                      # Fig. 2
    # non_infected_cc(df)                         # Fig. 3
    # diameter_by_closeness(df)                   # Fig. 4
